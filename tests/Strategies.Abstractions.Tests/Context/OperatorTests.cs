...
public class OperatorTests
{
    ...
    public void Constructor_WithIdAndIsAuthenticated_ShouldInitializeCorrectly()
    {
        ...
        // Assert
        op.ShouldSatisfyAllConditions(
            x => x.Id.ShouldBe("op123"),
            x => x.IsAuthenticated.ShouldBeTrue(),
            x => x.Name.ShouldBeNull(),
            x => x.Email.ShouldBeNull(),
            x => x.Roles.ShouldBeEmpty(),
            x => x.LoggableId.ShouldBe("op123"));
    }
    ...
    public void Constructor_WithIdRolesAndIsAuthenticated_ShouldInitializeCorrectly()
    {
        ...
        op.ShouldSatisfyAllConditions(
            x => x.Id.ShouldBe("op123"),
            x => x.IsAuthenticated.ShouldBeFalse(),
            x => x.Name.ShouldBeNull(),
            x => x.Email.ShouldBeNull(),
            x => x.Roles.Count.ShouldBe(2),
            x => x.Roles.ShouldContain("Admin"),
            x => x.Roles.ShouldContain("User"),
            x => x.LoggableId.ShouldBe("op123"));
    }
    ...
    public void Constructor_WithAllParameters_ShouldInitializeCorrectly()
    {
        ...
        // Assert
        op.ShouldSatisfyAllConditions(
            x => x.Id.ShouldBe("op456"),
            x => x.Name.ShouldBe("John Doe"),
            x => x.Email.ShouldBe("john@example.com"),
            x => x.Roles.ShouldContain("Manager"),
            x => x.IsAuthenticated.ShouldBeTrue(),
            x => x.LoggableId.ShouldBe("op456"));
    }
    ...
    public void Constructor_WithNullOrWhitespaceId_ShouldThrowArgumentException(string? invalidId)
    {
        ...
        Should.Throw<ArgumentException>(() => new Operator(invalidId!, [], true));
        Should.Throw<ArgumentException>(() => new Operator(invalidId!, "name", "email", [], true));
    }
    ...
    {
        // Arrange
        var op = new Operator("op123", ["Admin", "User"], true);

        // Act & Assert
        op.ShouldSatisfyAllConditions(
            x => x.HasRole("Admin").ShouldBeTrue(),
            x => x.HasRole("User").ShouldBeTrue());
    }
    ...
    {
        // Arrange
        var op = new Operator("op123", ["User"], true);
